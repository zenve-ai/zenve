...
from api.utils.github import commit_tree, list_tree_paths
...
class RepoWriterService:
    def scaffold_agent(
        ...
        if not workspace.github_installation_id or not workspace.github_repo or not workspace.github_default_branch:
    ...
    def delete_agent(self, workspace: Workspace, agent_slug: str, commit_message: str) -> str:
        ...
        paths = list_tree_paths(
            ...
            workspace.github_repo,
            prefix,
            ref=workspace.github_default_branch,
        )
        if not paths:
